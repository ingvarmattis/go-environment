...
ssh username@your-server-ip "echo 'SSH connection successful!'"
```
...
#### Добавление HOME_SERVER_IP

1. **Name**: `HOME_SERVER_IP`
2. **Value**: IP адрес вашего сервера (например, `192.168.1.100`)
3. Нажмите **Add secret**
...
1. Проверьте правильность IP адреса в `HOME_SERVER_IP`
...
- name: Deploy to server
  run: |
    ssh ${{ secrets.SSH_USER }}@${{ secrets.HOME_SERVER_IP }} << 'EOF'
...
export HOME_SERVER_IP="192.168.1.100"

# Тестируем подключение
ssh $SSH_USER@$HOME_SERVER_IP "echo 'Test successful'"
```
...
1. Обновите `HOME_SERVER_IP` в GitHub Secrets
2. Убедитесь, что новый IP доступен
